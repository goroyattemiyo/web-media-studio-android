...
/** The sole owner of WMS sound configuration; all calls except the processor callback use main. */
@UnstableApi
internal class SoundEngine(
    ...
    init {
        ...
        volume = AppVolumeState(saved, lastAudible, boostAvailable = processor.pcmSupported)
        ...
        val selected = if (savedSelection == "manual") null else savedSelection
            ?.takeIf { id -> (SoundPresets.builtIns + custom).any { it.id == id } } ?: "builtin:flat"
        eq = SoundEqState(
    ...
    private fun persistVolume() {
        preferences.edit()
            ...
            .putBoolean(KEY_BOOST, false) // never auto-arm after service recreation
            .apply()
    }
    ...
    private fun applyVolume() {
        ...
        processor.setBoostGain(
            if (requested > 100 && volume.boostEnabled && processor.pcmSupported && !eq.enabled)
                requested / 100f else 1f,
        )
        val expectedPlayerVolume = requested.coerceAtMost(100) / 100f
        if (abs(player.volume - expectedPlayerVolume) > 0.0001f) player.volume = expectedPlayerVolume
        persistVolume()
        publish()
    }
    ...
    private fun onPcmSupportChanged(supported: Boolean) {
        volume = volume.copy(boostAvailable = supported)
        if (!supported && volume.boostEnabled) {
            volume = volume.withBoost(false).copy(message = "この音声形式ではブーストできません。100%以下に戻しました。")
            applyVolume()
        } else publish()
    }
    ...
    private fun onRouteChanged() {
        ...
        volume = volume.withBoost(false).copy(message = "出力先の変更を検出したため、ブーストを解除しました。")
        applyVolume()
    }
    ...
    fun onAudioSessionIdChanged(newSessionId: Int) {
        ...
        try {
            candidate = Equalizer(0, newSessionId)
            check(candidate.hasControl()) { "EQの制御権がありません" }
            val count = candidate.numberOfBands.toInt()
            check(count in 1..32) { "対応帯域を取得できません" }
            val range = candidate.bandLevelRange
            check(range.size >= 2 && range[0] < range[1]) { "EQ調整幅が不正です" }
            equalizer = candidate
            val bands = (0 until count).map { index ->
                SoundBand((candidate.getCenterFreq(index.toShort()) / 1000).coerceAtLeast(1), 0)
            }
            eq = eq.copy(
                supported = true,
                bands = bands,
            ...
            eq = eq.copy(supported = false, enabled = false, bands = emptyList(),
                message = "この端末ではEQを利用できません: ${error.javaClass.simpleName}")
        }
        publish()
    }
    ...
    private fun setEqEnabled(enabled: Boolean): Int {
        ...
        if (enabled && volume.boostEnabled) return status("EQを使う前にブーストをOFFにしてください。")
        return try {
    ...
    private fun selectPreset(id: String): Int {
        ...
        if (volume.boostEnabled) return status("プリセットを使う前にブーストをOFFにしてください。")
        return try {
    ...
    private fun renamePreset(id: String, rawName: String): Int {
        val old = eq.customPresets.firstOrNull { it.id == id } ?: return status("変更できるマイプリセットがありません。")
    ...
    private fun deletePreset(id: String): Int {
        ...
        eq = eq.copy(customPresets = revised, selectedPresetId =
            if (eq.selectedPresetId == id) null else eq.selectedPresetId,
    ...
    fun execute(action: String, args: Bundle): Int = when (action) {
        SoundCommand.VOLUME -> {
            ...
            if (requested !in 0..volume.maximum) status("ブーストをONにしてから100%を超える音量を指定してください。")
        ...
        SoundCommand.BOOST -> {
            ...
            when {
                enabled && !processor.pcmSupported -> status("PCM出力が確認できないためブーストできません。")
                enabled && eq.enabled -> status("ブーストを使う前にEQをOFFにしてください。")
                else -> {
