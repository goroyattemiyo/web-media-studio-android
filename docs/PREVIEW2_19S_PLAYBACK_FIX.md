...
Date: 2026-09-17 (JST)
Status: Fix candidate committed; build, unit tests and real-device verification NOT YET RUN.
...
2. Run local unit tests, lint and debug build using the repository's `scripts/local-verify.ps1`. Do not use GitHub Actions for debugging.
...
Acceptance is OPEN until local checks and a real-device reproduction/retest pass. Do not merge this branch or publish Preview 2 automatically.
